...
export class GPUProfiler extends System {
  ...
  updateSettings(settings) {
    try {
      console.log('🔧 Updating GPU Profiler settings:', settings)
      
      // Update overlay visibility
      if (settings.overlay !== undefined && settings.overlay !== this.config.overlayEnabled) {
        this.config.overlayEnabled = settings.overlay
        console.log(`GPU profiler overlay: ${settings.overlay ? 'enabled' : 'disabled'}`)
        this.updateOverlayVisibility()
      }
      
      // Update auto optimization
      if (settings.autoOptimization !== undefined && settings.autoOptimization !== this.config.autoOptimization) {
        this.config.autoOptimization = settings.autoOptimization
        console.log(`Auto optimization: ${settings.autoOptimization ? 'enabled' : 'disabled'}`)
      }
      
      // Update performance warnings
      if (settings.warnings !== undefined && settings.warnings !== this.config.performanceWarnings) {
        this.config.performanceWarnings = settings.warnings
        console.log(`Performance warnings: ${settings.warnings ? 'enabled' : 'disabled'}`)
      }
      
      // Update thermal throttling detection
      if (settings.thermal !== undefined && settings.thermal !== this.config.thermalThrottling) {
        this.config.thermalThrottling = settings.thermal
        console.log(`Thermal throttling detection: ${settings.thermal ? 'enabled' : 'disabled'}`)
      }
      
    } catch (error) {
      console.warn('⚠️ Failed to update GPU Profiler settings:', error)
    }
  }
  ...
  updateOverlayVisibility() {
    if (this.overlays.canvas) {
      this.overlays.canvas.style.display = this.config.overlayEnabled ? 'block' : 'none'
      
      // If overlay is enabled, ensure it's updated with current data
      if (this.config.overlayEnabled) {
        this.updateOverlayContent()
        console.log('📊 GPU Profiler overlay now visible')
      }
    }
  }
  
  // NEW METHOD: Actually update overlay with real performance data
  updateOverlayContent() {
    if (!this.overlays.canvas) return
    
    const ctx = this.overlays.canvas.getContext('2d')
    if (!ctx) return
    
    // Clear canvas
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'
    ctx.fillRect(0, 0, this.overlays.canvas.width, this.overlays.canvas.height)
    ...
    ctx.fillText(`FPS: ${stats.fps.toFixed(1)}`, 10, y)
    y += 20
    ctx.fillText(`Frame Time: ${stats.frameTime.toFixed(2)}ms`, 10, y)
    y += 20
    ctx.fillText(`GPU Memory: ${stats.gpuMemory}MB`, 10, y)
    y += 20
    ctx.fillText(`Draw Calls: ${stats.drawCalls}`, 10, y)
    y += 20
    ctx.fillText(`Triangles: ${stats.triangles}`, 10, y)
    
    // Add AAA system stats if available
    if (this.world.graphics?.advancedMaterials) {
      y += 10
      ctx.fillStyle = '#ffff00'
      ctx.fillText('AAA Systems:', 10, y)
      y += 20
      ctx.fillStyle = '#00ff00'
      ctx.fillText(`Materials Enhanced: ${stats.materialsEnhanced || 0}`, 10, y)
      y += 20
      ctx.fillText(`Lights Enhanced: ${stats.lightsEnhanced || 0}`, 10, y)
    }
    
    // Show warnings if performance is poor
    if (stats.frameTime > 20) {
      y += 10
      ctx.fillStyle = '#ff0000'
      ctx.fillText('⚠️ Performance Warning', 10, y)
    }
  }
  ...
  getCurrentStats() {
    ...
    return {
      fps: this.performanceHistory.length > 0 ? 1000 / this.performanceHistory[this.performanceHistory.length - 1].frameTime : 0,
      frameTime: this.frameData.length > 0 ? this.frameData[this.frameData.length - 1].frameTime : 0,
      gpuMemory: info?.memory?.geometries ? Math.round((info.memory.geometries + info.memory.textures) / 1024 / 1024) : 0,
      drawCalls: info?.render?.calls || 0,
      triangles: info?.render?.triangles || 0,
      materialsEnhanced: this.frameData.length > 0 ? this.frameData[this.frameData.length - 1].materialsEnhanced : 0,
      lightsEnhanced: this.frameData.length > 0 ? this.frameData[this.frameData.length - 1].lightsEnhanced : 0
    }
  }
